...
$("printReceiptButton").addEventListener("click", imprimirReciboComNomeArquivo);
...
function imprimirReciboComNomeArquivo() {
  const numeroRecibo = limparTextoParaNomeArquivo(fields.printReceiptNumber.textContent) || "000";
  const recebiDe = limparTextoParaNomeArquivo(fields.printRecebiDe.textContent) || "Sem nome";
  const dataRecibo = limparTextoParaNomeArquivo(fields.printDataRecibo.textContent.replace(/\//g, "-")) || "Sem data";

  const nomeArquivo = `Recibo Nº ${numeroRecibo} - ${recebiDe} - ${dataRecibo}`;
...
function limparTextoParaNomeArquivo(texto) {
  return String(texto || "")
    .replace(/[\/:*?"<>|]/g, "")
    .replace(/\s+/g, " ")
    .trim()
